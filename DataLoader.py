...
class REFIT_train(Dataset):
    def __init__(self, filename='./REFIT_train.csv', n_samples=200, oneD=False):
        data_train = pd.read_csv(filename, header=None)
        data_train = data_train.apply(pd.to_numeric, errors='coerce').fillna(0).astype(int)
        # making the class labels for our dataset

        last_col_index = data_train.shape[1] - 1
        ...
        if oneD:
            ...
            self.X_train = self.X_train.reshape(self.X_train.shape[0], 1, 1, self.X_train.shape[1])
        self.y_train = train_dataset[last_col_index].values
...
class REFIT_test(Dataset):
    def __init__(self, filename='./mitbih_test.csv', n_samples=1000, oneD=False):
        ...
        if oneD:
            ...
            self.X_test = self.X_test.reshape(self.X_test.shape[0], 1, 1, self.X_test.shape[1])
        self.y_test = test_dataset[last_col_index].values
